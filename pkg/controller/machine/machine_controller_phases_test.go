...
func TestReconcilePhase(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			changed, err := r.reconcile(context.Background(), tc.machine)
			if tc.expectError {
...
func TestReconcileBootstrap(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			out := r.reconcileBootstrap(context.Background(), tc.machine)
			if tc.expectError {
				g.Expect(out.Error).ToNot(gomega.BeNil())
			} else {
				g.Expect(out.Error).To(gomega.BeNil())
			}

			if tc.expectChanged {
				g.Expect(out.Changed).To(gomega.BeTrue())
			} else {
				g.Expect(out.Changed).To(gomega.BeFalse())
			}
...
func TestReconcileInfrastructure(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			out := r.reconcileInfrastructure(context.Background(), tc.machine)
			if tc.expectError {
				g.Expect(out.Error).ToNot(gomega.BeNil())
			} else {
				g.Expect(out.Error).To(gomega.BeNil())
